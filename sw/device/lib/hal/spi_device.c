...
{
    /* clear interrupts enables, interrupts, and FIFOs */
    spi_device_interrupt_enable_set(spi_device, spi_device_intr_none);
    // TODO: Clang-tidy seems to be broken here, as this is a flag enum.
    // NOLINTBEGIN(clang-analyzer-optin.core.EnumCastOutOfRange)
    spi_device_intr intrs =
        spi_device_intr_upload_cmdfifo_not_empty | spi_device_intr_upload_payload_not_empty |
        spi_device_intr_upload_payload_overflow | spi_device_intr_readbuf_watermark |
        spi_device_intr_readbuf_flip | spi_device_intr_tpm_rdfifo_cmd_end |
        spi_device_intr_tpm_rdfifo_drop;
    // NOLINTEND(clang-analyzer-optin.core.EnumCastOutOfRange)
    spi_device_interrupt_clear(spi_device, intrs);
    spi_device_read_buffer_clear(spi_device);
    spi_device_flash_status_fifo_clear(spi_device);

    /* Write JEDEC ID data */
    spi_device_jedec_cc_write(spi_device, lowrisc_jedec_cc);
    spi_device_jedec_id_write(spi_device, lowrisc_jedec_id);
    ...
    spi_device_flash_status_write(spi_device, status);
    ...
    spi_device_cmd_info_write(spi_device, command_slot_0, 0u);
    ...
    spi_device_cmd_info_write(spi_device, command_slot_3, 3u);
    ...
    spi_device_cmd_info_write(spi_device, command_slot_4, 4u);
    ...
    spi_device_cmd_info_write(spi_device, command_slot_5, 5u);
    ...
    spi_device_cmd_info_write(spi_device, command_slot_11, 11u);
    ...
    spi_device_cmd_info_write(spi_device, command_slot_12, 12u);
    ...
    spi_device_cmd_info_write(spi_device, command_info_13, 13u);
    ...
    spi_device_cmd_info_write(spi_device, command_info_14, 14u);

    /* Slot 15: page program 4b */
    spi_device_cmd_info command_info_15 = {
        .opcode = spi_device_opcode_page_program_4b,
        .addr_mode = spi_device_addr_mode_4b,
        .upload = true,
        .busy = true,
        .valid = true,
    };
    spi_device_cmd_info_write(spi_device, command_info_15, 15u);

    /* enable write enable and write disable commands */
    spi_device_cmd_info_wren_write(spi_device, spi_device_opcode_write_enable, true);
    spi_device_cmd_info_wrdi_write(spi_device, spi_device_opcode_write_disable, true);
    /* enable 4b addressing mode commands */
    spi_device_cmd_info_en4b_write(spi_device, spi_device_opcode_enter_4b, true);
    spi_device_cmd_info_ex4b_write(spi_device, spi_device_opcode_exit_4b, true);

    /* enable 4b address mode */
...
spi_device_intr spi_device_interrupt_enable_read(spi_device_t spi_device)
{
...
spi_device_flash_status spi_device_flash_status_read(spi_device_t spi_device)
{
    return VOLATILE_READ(spi_device->flash_status);
}

void spi_device_flash_status_write(spi_device_t spi_device, spi_device_flash_status status)
{
    VOLATILE_WRITE(spi_device->flash_status, status);
}

void spi_device_flash_status_busy_write(spi_device_t spi_device, bool busy)
{
...
void spi_device_flash_status_wel_write(spi_device_t spi_device, bool wel)
{
    spi_device_flash_status status = VOLATILE_READ(spi_device->flash_status);
    status.wel = wel;
    VOLATILE_WRITE(spi_device->flash_status, status);
...
void spi_device_read_buffer_clear(spi_device_t spi_device)
{
    spi_device_control ctrl = VOLATILE_READ(spi_device->control);
    ctrl.flash_read_buffer_clr = true;
    VOLATILE_WRITE(spi_device->control, ctrl);
}

void spi_device_flash_status_fifo_clear(spi_device_t spi_device)
{
    spi_device_control ctrl = VOLATILE_READ(spi_device->control);
    ctrl.flash_status_fifo_clr = true;
    VOLATILE_WRITE(spi_device->control, ctrl);
}

bool spi_device_4b_addr_mode_enable_read(spi_device_t spi_device)
{
    spi_device_addr_mode mode = VOLATILE_READ(spi_device->addr_mode);
    return mode.addr_4b_en;
}

void spi_device_4b_addr_mode_enable_write(spi_device_t spi_device, bool enable)
{
    spi_device_4b_addr_mode_enable_write_unchecked(spi_device, enable);
...
void spi_device_4b_addr_mode_enable_write_unchecked(spi_device_t spi_device, bool enable)
{
    spi_device_addr_mode mode = { .addr_4b_en = enable };
    VOLATILE_WRITE(spi_device->addr_mode, mode);
}

spi_device_cmd_info spi_device_cmd_info_read(spi_device_t spi_device, size_t index)
{
...
void spi_device_cmd_info_write(spi_device_t spi_device, spi_device_cmd_info cmd_info, size_t index)
{
    if (index >= ARRAY_LEN(spi_device->cmd_info)) {
        return;
    }
...
spi_device_cmd_info_wren spi_device_cmd_info_wren_read(spi_device_t spi_device)
{
    return VOLATILE_READ(spi_device->cmd_info_wren);
}

void spi_device_cmd_info_wren_write(spi_device_t spi_device, uint8_t opcode, bool valid)
{
...
spi_device_cmd_info_wrdi spi_device_cmd_info_wrdi_read(spi_device_t spi_device)
{
    return VOLATILE_READ(spi_device->cmd_info_wrdi);
}

void spi_device_cmd_info_wrdi_write(spi_device_t spi_device, uint8_t opcode, bool valid)
{
...
spi_device_cmd_info_en4b spi_device_cmd_info_en4b_read(spi_device_t spi_device)
{
    return VOLATILE_READ(spi_device->cmd_info_en4b);
}

void spi_device_cmd_info_en4b_write(spi_device_t spi_device, uint8_t opcode, bool valid)
{
...
spi_device_cmd_info_ex4b spi_device_cmd_info_ex4b_read(spi_device_t spi_device)
{
    return VOLATILE_READ(spi_device->cmd_info_ex4b);
}

void spi_device_cmd_info_ex4b_write(spi_device_t spi_device, uint8_t opcode, bool valid)
{
    spi_device_cmd_info_ex4b ex4b = {
        .opcode = opcode,
        .valid = valid,
    };
    VOLATILE_WRITE(spi_device->cmd_info_ex4b, ex4b);
}

uint32_t spi_device_cmd_filter_read(spi_device_t spi_device, size_t index)
{
...
void spi_device_cmd_filter_write(spi_device_t spi_device, size_t index, uint32_t filter)
{
    if (index >= ARRAY_LEN(spi_device->cmd_filter)) {
        return;
    }
...
void spi_device_cmd_filter_set(spi_device_t spi_device, size_t index, uint32_t filter)
{
    if (index >= ARRAY_LEN(spi_device->cmd_filter)) {
        return;
    }
...
void spi_device_cmd_filter_clear(spi_device_t spi_device, size_t index, uint32_t filter)
{
    if (index >= ARRAY_LEN(spi_device->cmd_filter)) {
        return;
    }
...
spi_device_jedec_cc spi_device_jedec_cc_read(spi_device_t spi_device)
{
    return VOLATILE_READ(spi_device->jedec_cc);
}

void spi_device_jedec_cc_write(spi_device_t spi_device, spi_device_jedec_cc jedec_cc)
{
    VOLATILE_WRITE(spi_device->jedec_cc, jedec_cc);
}

spi_device_jedec_id spi_device_jedec_id_read(spi_device_t spi_device)
{
    return VOLATILE_READ(spi_device->jedec_id);
}

void spi_device_jedec_id_write(spi_device_t spi_device, spi_device_jedec_id jedec_id)
{
    VOLATILE_WRITE(spi_device->jedec_id, jedec_id);
}

spi_device_upload_status spi_device_upload_status_read(spi_device_t spi_device)
{
    return VOLATILE_READ(spi_device->upload_status);
}

spi_device_upload_status2 spi_device_upload_status2_read(spi_device_t spi_device)
{
    return VOLATILE_READ(spi_device->upload_status2);
}

spi_device_upload_cmdfifo spi_device_upload_cmdfifo_read(spi_device_t spi_device)
{
...
uint32_t spi_device_mailbox_addr_read(spi_device_t spi_device)
{
    return VOLATILE_READ(spi_device->mailbox_addr);
}

void spi_device_mailbox_addr_write(spi_device_t spi_device, uint32_t mailbox_addr)
{
    ...
    spi_device_t spi_device, spi_device_software_command *command)
{
    ...
    command->opcode = spi_device_upload_cmdfifo_read(spi_device).data;

    if (spi_device_upload_status_read(spi_device).addrfifo_notempty) {
        command->has_address = true;
        command->address = spi_device_upload_addrfifo_read(spi_device);
    } else {
        /* no address */
        command->has_address = false;
        command->address = 0u;
    }
    command->payload_byte_count = spi_device_upload_status2_read(spi_device).payload_depth;
