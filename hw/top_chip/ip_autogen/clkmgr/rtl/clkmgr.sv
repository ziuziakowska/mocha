...
#(
  ...
  ) u_clk_main_hint_trans (
    .clk_i(clk_main),
    .clk_gated_i(clk_main_root),
    .rst_ni(rst_main_ni),
    .en_i(clk_main_en),
    .idle_i(idle_i[HintMainHint]),
    .sw_hint_i(reg2hw.clk_hints.q),
    .scanmode_i,
    .alert_cg_en_o(cg_en_o.main_hint),
    .clk_o(clocks_o.clk_main_hint),
    .clk_reg_i(clk_i),
    .rst_reg_ni(rst_ni),
    .reg_en_o(hw2reg.clk_hints_status.d),
    .reg_cnt_err_o(idle_cnt_err[HintMainHint])
  );
  `ASSERT_PRIM_COUNT_ERROR_TRIGGER_ALERT(
    ClkMainHintCountCheck_A,
    u_clk_main_hint_trans.u_idle_cnt,
    alert_tx_o[1])
